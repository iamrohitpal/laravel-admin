                            ...
                            <div class="col-12">
                                ...
                                <select name="permission_type" class="single-select form-control" required>
                                    @foreach ($data['type'] as $key => $role)
                                        @php
                                            $selected = !empty($data['role']['permission_type'])
                                                ? $data['role']['permission_type']
                                                : '';
                                        @endphp
                                        <option value="{{ $key }}"
                                            {{ $selected == $key || $key == old('permission_type') ? 'selected' : '' }}>
                                            {{ $role }}</option>
                                    @endforeach
                                </select>
                            </div>
                                    ...
                                    @foreach ($data['permission'] as $permission)
                                        <tr style="border-bottom: 1px solid #ced4da;line-height: 40px;">
                                            ...
                                            @foreach ($permission['permission'] as $role)
                                                <td><input id="permissions{{ $role['id'] }}" name="permissions[]"
                                                        value="{{ $role['id'] }}" type="checkbox"
                                                        {{ !empty($data['role']['permissions']) ? (in_array($role['id'], explode(',', $data['role']['permissions'])) ? 'checked' : '') : '' }}>
                                                    <label
                                                        for="permission_id{{ $role['id'] }}">{{ $role['name'] }}</label>
                                                </td>
                                            @endforeach
                                        </tr>
                                    @endforeach
